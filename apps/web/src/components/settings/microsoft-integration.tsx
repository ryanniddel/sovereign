...
export function MicrosoftIntegration({ status }: MicrosoftIntegrationProps) {
  const connect = useConnectIntegration();
  const disconnect = useDisconnectIntegration();
  const isConnected = status?.connected ?? false;
  const { data: calendars, isLoading: calendarsLoading } = useMicrosoftCalendars(isConnected);

  return (
    <IntegrationCard
      name="Microsoft"
      icon="M"
      description="Outlook Calendar, Email, and Teams"
      connected={isConnected}
      accountEmail={status?.accountEmail}
      status={status?.status ?? 'DISCONNECTED'}
      onConnect={() => connect.mutate('MICROSOFT')}
      onDisconnect={() => disconnect.mutate('MICROSOFT')}
    ...
    >
      <div className="space-y-3">
        ...
        {/* Teams meeting link status */}
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">Teams Meeting Links</span>
