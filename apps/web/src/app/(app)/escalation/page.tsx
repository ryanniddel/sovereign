...
import { Plus, Copy, BarChart3, Link2 } from 'lucide-react';
import Link from 'next/link';
import { useEscalationRules, useCloneEscalationRule } from '@/hooks/use-escalation';
...
export default function EscalationPage() {
  ...
  const clone = useCloneEscalationRule();
    ...
    { key: 'isActive', header: 'Status', cell: (r) => <Badge variant={r.isActive ? 'default' : 'secondary'}>{r.isActive ? 'Active' : 'Inactive'}</Badge> },
    {
      key: 'actions', header: '', cell: (r) => (
        <Button
          ...
          variant="ghost"
          className="h-8 w-8"
          onClick={(e) => { e.stopPropagation(); clone.mutate(r.id); }}
          title="Clone rule"
        >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" asChild>
            <Link href="/escalation/analytics"><BarChart3 className="mr-1 h-4 w-4" />Analytics</Link>
          </Button>
          <Button variant="outline" asChild>
            <Link href="/escalation/active-chains"><Link2 className="mr-1 h-4 w-4" />Active Chains</Link>
          </Button>
          <Button variant="outline" asChild>
            <Link href="/escalation/logs">View Logs</Link>
          </Button>
          <Button asChild>
            <Link href="/escalation/new"><Plus className="mr-1 h-4 w-4" />New Rule</Link>
          </Button>
        </div>
      </div>
